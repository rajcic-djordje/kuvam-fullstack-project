...
export class LocationModal implements OnInit {
  ...
  readonly mode =
    input<LocationModalMode>(
      'setup'
    );

  readonly confirmCityChange =
  input(false);
  ...
  saveLocation(): void {
    ...
    const values =
      this.locationForm
        ...
        if (
          hasCityChanged &&
          this.confirmCityChange()
        ) {
          const confirmed =
            window.confirm(
              'Promenom grada trenutne stavke u korpi više možda neće biti dostupne. Ako nastaviš, korpa će biti ispražnjena. Da li želiš da nastaviš?'
            );
